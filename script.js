...
async function fetchNews(query) {
    try {
        // Fetch from News API
        const newsRes = await fetch(`${NEWS_API_URL}${encodeURIComponent(query)}&apiKey=${NEWS_API_KEY}`);
        if (!newsRes.ok) throw new Error("Failed to fetch news");
        const newsData = await newsRes.json();
        bindData(newsData.articles);

        // Fetch from Crunchbase API
        const crunchbaseRes = await fetch(`${CRUNCHBASE_URL}${encodeURIComponent(query)}&user_key=${CRUNCHBASE_API_KEY}`);
        if (!crunchbaseRes.ok) throw new Error("Failed to fetch data from Crunchbase");
        const crunchbaseData = await crunchbaseRes.json();
        bindData(crunchbaseData.data.items);

        // Fetch from AngelList API
        const angellistRes = await fetch(`${ANGELLIST_URL}${encodeURIComponent(query)}&access_token=${ANGELLIST_API_KEY}`);
        if (!angellistRes.ok) throw new Error("Failed to fetch data from AngelList");
        const angellistData = await angellistRes.json();
        bindData(angellistData.startups);

        // Fetch from LinkedIn API
        const linkedinRes = await fetch(`${LINKEDIN_URL}${encodeURIComponent(query)}&oauth2_access_token=${LINKEDIN_API_KEY}`);
        if (!linkedinRes.ok) throw new Error("Failed to fetch data from LinkedIn");
...
function fillDataInCard(cardClone, article) {
    ...
    newsTitle.textContent = `${article.title.slice(0, 60)}...`;
    newsDesc.textContent = `${article.description.slice(0, 150)}...`;
